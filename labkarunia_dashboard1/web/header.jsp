...
--%>
<script>
    var lab_user_id = window.localStorage.getItem('lab_user_id');
    var userlevel = window.localStorage.getItem('userlevel');
    var user_name = window.localStorage.getItem('lab_user_name');


    jQuery(document).ready(function ($) {
        if (lab_user_id === null || lab_user_id === '') {
            window.location.href = "ho";
        }

//        if (userlevel === 'owner') {
//            $.getJSON('opt?action=option_outlet', {brand_id: brand_id}, function (data) {
//                var options = '<option selected="true" disabled value="">Select Branch</option>';
...
//        if (userlevel === 'owner') {
//            $.getJSON('opt?action=option_outlet', {brand_id: brand_id}, function (data) {
//                var options = '<option selected="true">All Branch</option>';
//                for (var i = 0; i < data.length; i++) {
//                    options += '<option value="' + data[i].outlet_id + '">' + data[i].outlet_name + '</option>';
//                }
...
<script>
    jQuery(document).ready(function ($) {
        ...
        switch (userlevel) {
            case '1':
                userlevelnya = 'administrator';
                break;
            case '2':
                userlevelnya = 'operasional';
                break;
            case '3':
                userlevelnya = 'analis';
                break;
        ...
        document.getElementById("role").innerHTML = 'Role : ' + userlevelnya;
        ...
        $('#yes').on('click', function (event) {
            ...
            window.localStorage.removeItem('lab_user_id');
            window.localStorage.removeItem('side');
            window.localStorage.removeItem('lab_user_name');
            window.location.href = "ho";
        });
