 ...
 * @author suhanda
 */
public class getlistsalesman {

    private static Logger log = Logger.getLogger(getlistsalesman.class);

    public HashMap process(HashMap input) {
        BackendDBProcess dp = new BackendDBProcess();
        Gson gson = new Gson();
        ArrayList<salesman> listcustomer = dp.getlistsalesman();
        try {
            if (listcustomer != null) {
                JsonElement element = gson.toJsonTree(listcustomer);
                input.put("listsalesman", element);
