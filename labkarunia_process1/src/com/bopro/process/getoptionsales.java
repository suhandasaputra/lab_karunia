 ...
 * @author suhanda
 */
public class getoptionsales {

    private static Logger log = Logger.getLogger(getoptionsales.class);

    public HashMap process(HashMap input) {
        BackendDBProcess dp = new BackendDBProcess();
        Gson gson = new Gson();
        ArrayList<getsales> listsupplier = dp.getoptionsales();
        try {
            if (listsupplier != null) {
                JsonElement element = gson.toJsonTree(listsupplier);

                input.put("listsales", element);
