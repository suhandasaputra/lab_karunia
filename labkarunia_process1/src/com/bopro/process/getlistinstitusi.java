 ...
 * @author suhanda
 */
public class getlistinstitusi {

    private static Logger log = Logger.getLogger(getlistinstitusi.class);

    public HashMap process(HashMap input) {
        BackendDBProcess dp = new BackendDBProcess();
        Gson gson = new Gson();
        ArrayList<institusi> listcustomer = dp.getlistinstitusi();
        try {
            if (listcustomer != null) {
                JsonElement element = gson.toJsonTree(listcustomer);
                input.put("listinstitusi", element);
