 ...
 * @author suhanda
 */
public class getlisttipem {

    private static Logger log = Logger.getLogger(getlisttipem.class);

    public HashMap process(HashMap input) {
        BackendDBProcess dp = new BackendDBProcess();
        Gson gson = new Gson();
        ArrayList<tipe_pemabayaran> listcustomer = dp.getlisttipem();
        try {
            if (listcustomer != null) {
                JsonElement element = gson.toJsonTree(listcustomer);
                input.put("listtipem", element);
